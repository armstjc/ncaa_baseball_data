...
def get_baseball_teams(season: int, level: str | int) -> pd.DataFrame:
    ...
    if "but something went wrong (500)" in response:
        url = (
            "https://stats.ncaa.org/rankings/national_ranking?"
            + f"academic_year={season}.0&division={ncaa_level}.0&"
            + f"ranking_period={rp_value}&sport_code={sport_id}"
...
def get_baseball_team_roster(team_id: int) -> pd.DataFrame:
    ...
    if table is None:
        table = soup.find_all(
            "table",
            {"class": "dataTable small_font no_padding no-footer"}
        )[-1]
...
def get_baseball_game_player_stats(game_id: int) -> pd.DataFrame:
    ...
    if "Box score not available" in response:
        return stats_df
    ...
    if len(away_url) > 0:
        ...
        away_team_name = team_cards[0].text
        away_team_id = -1
...
def get_raw_baseball_game_pbp(game_id: int):
    ...
    if "Play by play not available" in response:
        return pbp_df
    ...
    if len(pbp_df_arr) == 0:
        logging.warning(
            f"No PBP data found in game ID #{game_id}"
        )
        return pbp_df
